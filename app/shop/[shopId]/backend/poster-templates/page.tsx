...
interface PageProps {
  ...
  searchParams: Promise<{ category?: string; orientation?: string; difficulty?: string; status?: string; search?: string }>;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    console.error('Error generating metadata for poster templates page:', error);
    return {
...
export default async function PosterTemplatesPage({ params, searchParams }: PageProps) {
  const { shopId } = await params;
  const { category, orientation, difficulty, status, search } = await searchParams;
  const presenter = await PosterTemplatesPresenterFactory.create();
      ...
      <BackendLayout shopId={shopId}>
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">ไม่สามารถโหลดข้อมูลเทมเพลตโปสเตอร์ได้</p>
            <button
