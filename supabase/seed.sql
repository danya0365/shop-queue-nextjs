...
-- Insert Employees auth.users match email somchai@stylehair.com
...
-- Insert Employees auth.users match email somsri@stylehair.com
...
FROM profiles p
JOIN auth.users u ON p.auth_id = u.id
WHERE u.email = 'shop_owner@example.com' AND p.username = 'shop_owner'
LIMIT 1;
...
AND u.email = 'shop_owner@example.com'
AND p.username = 'shop_owner';
