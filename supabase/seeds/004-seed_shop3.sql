...
redeemed_transactions AS (
  ...
  SELECT
    ...
    CASE 
      WHEN cd.name = 'นายเทค โซลูชั่น' THEN -200  -- Redeem 200 points for free screen protector
      WHEN cd.name = 'นางสาวแอป โมบาย' THEN -100  -- Redeem 100 points for 10% discount
    END AS points,
    CASE 
      WHEN cd.name = 'นายเทค โซลูชั่น' THEN 'แลกรางวัลฟิล์มกันรอยฟรี'
      WHEN cd.name = 'นางสาวแอป โมบาย' THEN 'แลกรางวัลส่วนลด 10%'
    END AS description,
    qd.queue_id,
    CASE 
      WHEN cd.name = 'นายเทค โซลูชั่น' THEN '{"reward_type": "free_item", "item": "screen_protector"}'::jsonb
      WHEN cd.name = 'นางสาวแอป โมบาย' THEN '{"reward_type": "discount", "discount_percent": 10}'::jsonb
    END AS metadata,
    NOW() - INTERVAL '4 days' AS transaction_date,
    NOW() - INTERVAL '4 days' AS created_at
  FROM customer_data cd
  JOIN queue_data qd ON cd.customer_id = qd.customer_id
  WHERE cd.name IN ('นายเทค โซลูชั่น', 'นางสาวแอป โมบาย')
  LIMIT 2
...
WHERE (ABS(rt.points) = 200 AND rd.name = 'ส่วนลด 20%') 
