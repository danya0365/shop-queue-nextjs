...
redeemed_transactions AS (
  ...
  SELECT
    ...
    CASE 
      WHEN cd.name = 'นายคลีน เสื้อผ้า' THEN -150  -- Redeem 150 points for free wash
      WHEN cd.name = 'นางสาวเฟรช ซักผ้า' THEN -100  -- Redeem 100 points for 10% discount
    END AS points,
    CASE 
      WHEN cd.name = 'นายคลีน เสื้อผ้า' THEN 'แลกรางวัลซักฟรี 1 ครั้ง'
      WHEN cd.name = 'นางสาวเฟรช ซักผ้า' THEN 'แลกรางวัลส่วนลด 10%'
    END AS description,
    qd.queue_id,
    CASE 
      WHEN cd.name = 'นายคลีน เสื้อผ้า' THEN '{"reward_type": "free_item", "item": "wash_service"}'::jsonb
      WHEN cd.name = 'นางสาวเฟรช ซักผ้า' THEN '{"reward_type": "discount", "discount_percent": 10}'::jsonb
    END AS metadata,
  ...
  WHERE cd.name IN ('นายคลีน เสื้อผ้า', 'นางสาวเฟรช ซักผ้า')
  LIMIT 2
...
WHERE (ABS(rt.points) = 150 AND rd.name = 'ส่วนลด 20%') 
   OR (ABS(rt.points) = 100 AND rd.name = 'รีดฟรี 1 ชิ้น');
