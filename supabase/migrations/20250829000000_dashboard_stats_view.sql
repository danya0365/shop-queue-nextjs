...
BEGIN
    ...
    SELECT 
        ...
        ps.rank_in_category
    FROM 
        popular_services_by_category_view ps
    WHERE 
        ps.shop_id = p_shop_id
        AND ps.category = p_category
    ORDER BY 
        ps.rank_in_category
    LIMIT p_limit;
END;
...
GRANT EXECUTE ON FUNCTION get_customer_popular_services(UUID, INTEGER, TEXT) TO anon;
...
COMMENT ON FUNCTION get_customer_popular_services(UUID, INTEGER, TEXT) IS 'Get popular services for a specific shop with optional category filter';
