...
export class PostersPresenter extends BaseShopBackendPresenter {
  ...
  async getViewModel(shopId: string): Promise<PostersViewModel> {
    try {
      this.logger.info(
        `PostersPresenter: Getting view model for shop ${shopId}`
      );
  ...
  private async getUserSubscription(
    ...
    const maxFreePosters =
      subscriptionPlan.tier === "free"
        ? 3
        : subscriptionPlan.tier === "pro"
        ? 10
        : 999999;
    ...
    const paidPostersUsed = Math.max(
      0,
      (usage.totalPosters || 0) - maxFreePosters
    );
...
export class ClientPostersPresenterFactory {
  static create(): PostersPresenter {
