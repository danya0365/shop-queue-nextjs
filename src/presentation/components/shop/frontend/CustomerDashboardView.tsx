  ...
  return (
    <div className="space-y-8">
        ...
        <div className="frontend-shop-header p-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="frontend-shop-header-text-light mb-4">{shopInfo.description}</p>
              <div className="flex items-center space-x-4 text-sm">
                ...
                <div className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs ${
                  shopInfo.isOpen ? 'frontend-status-open' : 'frontend-status-closed'
                }`}>
        ...
        {showQRCode && (
          <div className="p-6 frontend-qr-section">
            <div className="text-center">
              ...
              <p className="text-sm frontend-text-secondary">สแกน QR Code เพื่อเข้าคิวด่วน</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="frontend-card">
        <div className="p-6">
          <h2 className="text-xl font-semibold frontend-text-primary mb-6">สถานะคิวปัจจุบัน</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="w-16 h-16 frontend-queue-current rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl font-bold">{queueStatus.currentNumber}</span>
              </div>
            ...
            <div className="text-center">
              <div className="w-16 h-16 frontend-queue-waiting rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl font-bold">{queueStatus.totalWaiting}</span>
              </div>
            ...
            <div className="text-center">
              <div className="w-16 h-16 frontend-queue-time rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl font-bold">{queueStatus.estimatedWaitTime}</span>
              </div>
            ...
            <div className="text-center">
              <div className="w-16 h-16 frontend-queue-average rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl font-bold">{queueStatus.averageServiceTime}</span>
              </div>
              <p className="text-sm frontend-text-secondary">เวลาเฉลี่ย (นาที)</p>
            </div>
          </div>
      ...
      <div className="frontend-card">
        <div className="p-6 border-b">
          <h2 className="text-xl font-semibold frontend-text-primary">เมนูยอดนิยม</h2>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularServices.map((service) => (
              <div key={service.id} className="frontend-service-card rounded-lg p-4">
                <div className="text-center">
                  <span className="text-4xl mb-3 block">{service.icon}</span>
                  <h3 className="font-semibold frontend-text-primary mb-2">{service.name}</h3>
                  <p className="text-sm frontend-text-secondary mb-3">{service.description}</p>
      ...
      {promotions.length > 0 && (
        <div className="frontend-card">
          <div className="p-6 border-b">
            <h2 className="text-xl font-semibold frontend-text-primary">โปรโมชันพิเศษ</h2>
          </div>
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {promotions.map((promotion) => (
                <div key={promotion.id} className="frontend-promotion-card rounded-lg p-6">
                  <div className="flex items-start space-x-4">
                    <span className="text-3xl">{promotion.icon}</span>
                    <div className="flex-1">
                      <h3 className="font-semibold frontend-text-primary mb-2">{promotion.title}</h3>
                      <p className="text-sm frontend-text-secondary mb-3">{promotion.description}</p>
                      <div className="flex justify-between items-center">
                        <span className="frontend-promotion-badge px-3 py-1 rounded-full text-sm font-medium">
                          ลด {promotion.discount}%
                        </span>
                        <span className="text-xs frontend-text-muted">ถึง {promotion.validUntil}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div className="text-center">
            ...
            <h3 className="font-semibold frontend-text-primary mb-2">เข้าคิว</h3>
        ...
        >
          <div className="text-center">
            <span className="text-4xl mb-3 block">⏰</span>
            <h3 className="font-semibold frontend-text-primary mb-2">ติดตามคิว</h3>
        ...
        >
          <div className="text-center">
            <span className="text-4xl mb-3 block">🎁</span>
            <h3 className="font-semibold frontend-text-primary mb-2">แต้มสะสม</h3>
            <p className="text-sm frontend-text-secondary">ดูแต้มและสิทธิพิเศษ</p>
          </div>
        </Link>
      </div>
      ...
      <div className="frontend-card">
        <div className="p-6 border-b">
          <h2 className="text-xl font-semibold frontend-text-primary">ข้อมูลร้าน</h2>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="frontend-text-secondary">{shopInfo.openTime} - {shopInfo.closeTime}</p>
            </div>
          </div>
        </div>
      </div>
