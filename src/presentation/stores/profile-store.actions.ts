...
import { CreateProfileInputDto, ProfileDto, UpdateProfileInputDto } from '../../application/dtos/profile-dto';
...
export const createProfileActions = (
  ...
  fetchActiveProfile: async (authId: string): Promise<ProfileDto | null> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch active profile';
  ...
  createProfile: async (profile: CreateProfileInputDto): Promise<ProfileDto> => {
    setState({ loading: true, error: null });
    try {
      const newProfile = await profileAdapter.create(profile);
  ...
  updateProfile: async (id: string, profile: UpdateProfileInputDto): Promise<ProfileDto> => {
    setState({ loading: true, error: null });
    try {
      const updatedProfile = await profileAdapter.update(id, profile);
  ...
  switchProfile: async (id: string, authId: string): Promise<boolean> => {
    ...
    try {
      ...
      await profileAdapter.setActive(id, authId);
      ...
      const newActiveProfile = updatedProfiles.find(profile => profile.id === id) || null;
  ...
  deleteProfile: async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      await profileAdapter.delete(id);

      // Remove the deleted profile from the list
      const updatedProfiles = getState().profiles.filter(profile => profile.id !== id);

      setState({
        profiles: updatedProfiles,
        // If we deleted the active profile, set activeProfile to null
        activeProfile: getState().activeProfile?.id === id ? null : getState().activeProfile,
        loading: false
      });
