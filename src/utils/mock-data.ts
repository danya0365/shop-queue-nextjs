...
export const MOCK_SHOP_DATA: ShopCreateData = {
  ...
  description:
    "ร้านตัดผมชายที่ทันสมัย บริการตัดผม ทำผม และดูแลเส้นผมโดยช่างผมมืออาชีพ",
...
export const MOCK_RESTAURANT_DATA: ShopCreateData = {
  ...
  description:
    "ร้านอาหารไทยแท้ๆ บริการอาหารไทยภาคกลางและภาคอีสาน วัตถุดิบสดใหม่ทุกวัน",
  category: "restaurant",
...
export const MOCK_COFFEE_SHOP_DATA: ShopCreateData = {
  name: "คาเฟ่สาขลด",
  description:
    "คาเฟ่เล็กๆ สไตล์โฮมเมด บริการกาแฟสด เค้กและของว่างในบรรยากาศสบายๆ",
  category: "cafe",
...
export const getMockShopData = (): ShopCreateData => {
  ...
  const shopTypes = [
    {
      type: "haircut",
      names: [
        "ร้านตัดผมชายสมัยใหม่",
        "ร้านตัดผมสไตล์คลาสสิก",
        "ร้านตัดผมเกาหลี",
        "ร้านตัดผมญี่ปุ่น",
        "ร้านตัดผมยุโรป",
      ],
  ...
  const randomName =
    randomShopType.names[
      Math.floor(Math.random() * randomShopType.names.length)
    ];
  ...
  const randomPhone = `${randomPrefix}-${randomNumber
    .toString()
    .slice(0, 3)}-${randomNumber.toString().slice(3, 7)}`;

  // Random emails
  const emailDomains = ["gmail.com", "hotmail.com", "yahoo.com", "outlook.com"];
  const randomDomain =
    emailDomains[Math.floor(Math.random() * emailDomains.length)];
  const randomEmail = `contact${Math.floor(
    Math.random() * 1000
  )}@${randomDomain}`;

  // Random websites
  const websiteDomains = [".com", ".co.th", ".net", ".org"];
  const randomWebsiteDomain =
    websiteDomains[Math.floor(Math.random() * websiteDomains.length)];
  const randomWebsite = `https://${randomName
    .toLowerCase()
    .replace(/\s+/g, "")}${randomWebsiteDomain}`;
  ...
  const generateOperatingHours = () => {
    ...
    days.forEach((day) => {
      ...
      const breakStart = hasBreak
        ? `${Math.floor(Math.random() * 3) + 12}:00`
        : "12:00";
      const breakEnd = hasBreak
        ? `${Math.floor(Math.random() * 2) + 13}:00`
        : "13:00";
