 ...
 */
export class SupabaseBackendQueueRepository extends BackendRepository implements BackendQueueRepository {
  constructor(
  ...
  async getPaginatedQueues(params: PaginationParams): Promise<PaginatedQueuesEntity> {
    try {
      ...
      const pagination = SupabaseBackendQueueMapper.createPaginationMeta(page, limit, totalItems);
