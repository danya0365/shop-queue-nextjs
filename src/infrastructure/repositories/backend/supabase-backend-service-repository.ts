...
import { BackendServiceError, BackendServiceErrorType, BackendServiceRepository } from "../../../domain/repositories/backend/backend-service-repository";
import { SupabaseBackendServiceMapper } from "../../mappers/backend/supabase-backend-service.mapper";
import { ServiceSchemaType, ServiceStatsSchemaType } from "../../schemas/backend/service.schema";
 ...
 */
export class SupabaseBackendServiceRepository extends BackendRepository implements BackendServiceRepository {
  constructor(
    dataSource: DatabaseDataSource,
    logger: Logger
  ) {
    super(dataSource, logger, "BackendService");
  }
  ...
  async getPaginatedServices(params: PaginationParams & {
    ...
    try {
      ...
      const mappedServices = services.map(service => {
        ...
        return SupabaseBackendServiceMapper.toDomain(serviceWithShopName);
      });

      // Create pagination metadata
      const pagination = SupabaseBackendServiceMapper.createPaginationMeta(page, limit, totalItems);
      ...
      throw new BackendServiceError(
        BackendServiceErrorType.UNKNOWN,
  ...
  async createService(service: Omit<ServiceEntity, 'id' | 'createdAt' | 'updatedAt'>): Promise<ServiceEntity> {
    try {
      ...
      return SupabaseBackendServiceMapper.toDomain(createdService);
    } catch (error) {
      if (error instanceof BackendServiceError) {
        throw error;
      }
  ...
  async updateService(id: string, updates: Partial<ServiceEntity>): Promise<ServiceEntity> {
    try {
      const updateData = {
        ...SupabaseBackendServiceMapper.toDatabase(updates as ServiceEntity),
      ...
      if (!updatedService) {
        throw new BackendServiceError(
          BackendServiceErrorType.NOT_FOUND,
          'Service not found',
          'updateService',
          { id }
        );
      ...
      return SupabaseBackendServiceMapper.toDomain(updatedService);
    } catch (error) {
      if (error instanceof BackendServiceError) {
        throw error;
      }
  ...
  async getServiceStats(): Promise<ServiceStatsEntity> {
    try {
      ...
      return SupabaseBackendServiceMapper.statsToEntity(statsData[0]);
    } catch (error) {
      if (error instanceof BackendServiceError) {
        throw error;
      }
