import { CreateRewardEntity, PaginatedRewardsEntity, RewardEntity, RewardStatsEntity, RewardTypeStatsEntity, RewardUsageEntity } from "../../../domain/entities/backend/backend-reward.entity";
...
import { BackendRewardError, BackendRewardErrorType, BackendRewardRepository } from "../../../domain/repositories/backend/backend-reward-repository";
import { SupabaseBackendRewardMapper } from "../../mappers/backend/supabase-backend-reward.mapper";
import { RewardSchema, RewardStatsSchema, RewardUsageSchema } from "../../schemas/backend/reward.schema";
 ...
 */
export class SupabaseBackendRewardRepository extends BackendRepository implements BackendRewardRepository {
  constructor(
    dataSource: DatabaseDataSource,
    logger: Logger
  ) {
    super(dataSource, logger, "BackendReward");
  }
  ...
  async getPaginatedRewards(params: PaginationParams): Promise<PaginatedRewardsEntity> {
    try {
      ...
      const mappedRewards = rewards.map(reward => {
        ...
        return SupabaseBackendRewardMapper.toDomain(rewardWithJoins);
      });

      // Create pagination metadata
      const pagination = SupabaseBackendRewardMapper.createPaginationMeta(page, limit, totalItems);
  ...
  async getRewardStats(): Promise<RewardStatsEntity> {
    try {
      ...
      return SupabaseBackendRewardMapper.statsToEntity(statsData[0]);
    } catch (error) {
      if (error instanceof BackendRewardError) {
        throw error;
      }
  ...
  async createReward(reward: Omit<CreateRewardEntity, 'id' | 'createdAt' | 'updatedAt'>): Promise<RewardEntity> {
    try {
      ...
      if (!createdReward) {
        throw new BackendRewardError(
          BackendRewardErrorType.OPERATION_FAILED,
