 ...
 */
export class SupabaseBackendPaymentRepository extends BackendRepository implements BackendPaymentRepository {
  constructor(
    dataSource: DatabaseDataSource,
    logger: Logger
  ) {
    super(dataSource, logger, "BackendPayment");
  }
  ...
  async getPaginatedPayments(params: PaginationParams): Promise<PaginatedPaymentsEntity> {
    try {
      ...
      if (payments.length === 0) {
        ...
        const pagination = SupabaseBackendPaymentMapper.createPaginationMeta(page, limit, totalItems);
      ...
      const mappedPayments = payments.map(payment => {
        ...
        return SupabaseBackendPaymentMapper.toDomain(paymentWithJoins);
      });
  ...
  async getPaymentStats(): Promise<PaymentStatsEntity> {
    try {
      ...
      return SupabaseBackendPaymentMapper.statsToEntity(statsData[0]);
    } catch (error) {
      if (error instanceof BackendPaymentError) {
        throw error;
      }
