...
type ProfileSubscriptionSchemaRecord = Record<string, unknown> &
 ...
 */
export class SupabaseProfileSubscriptionRepository
  extends StandardRepository
  implements ProfileSubscriptionRepository
{
  ...
  async getPaginatedSubscriptions(
    ...
    profileId?: string
  ): Promise<PaginatedProfileSubscriptionsEntity> {
    try {
      ...
      const profileSubscriptions =
        await this.dataSource.getAdvanced<ProfileSubscriptionSchemaRecord>(
      ...
      const entities = profileSubscriptions.map(
        (subscription: ProfileSubscriptionSchema) =>
          SupabaseBackendSubscriptionMapper.profileSubscriptionSchemaToEntity(
            subscription
          )
      );

      const pagination = SupabaseBackendSubscriptionMapper.createPaginationMeta(
        page,
        limit,
        totalItems
      );
  ...
  async getSubscriptionById(
    id: string
  ): Promise<ProfileSubscriptionEntity | null> {
    try {
      ...
      return SupabaseBackendSubscriptionMapper.profileSubscriptionSchemaToEntity(
        results[0]
      );
    } catch (error) {
   ...
   */
  async getActiveSubscriptionByProfileId(
    profileId: string
  ): Promise<ProfileSubscriptionEntity | null> {
    try {
      const queryOptions: QueryOptions = {
   ...
   */
  async getSubscriptionHistoryByProfileId(
    profileId: string,
    params: PaginationParams
  ): Promise<PaginatedProfileSubscriptionsEntity> {
    try {
      this.logger.info("Getting subscription history by profile ID", {
        profileId,
        params,
      });
      ...
      this.logger.error("Error getting subscription history by profile ID", {
        profileId,
        params,
        error,
      });
  ...
  async createSubscription(
    subscription: CreateProfileSubscriptionEntity
  ): Promise<ProfileSubscriptionEntity> {
    try {
      this.logger.info("Creating profile subscription", { subscription });

      const subscriptionData =
        SupabaseBackendSubscriptionMapper.createProfileSubscriptionEntityToSchema(
          subscription
        );

      const result =
        await this.dataSource.insert<ProfileSubscriptionSchemaRecord>(
          "profile_subscriptions",
          subscriptionData
        );
  ...
  async updateSubscription(
    id: string,
    subscription: Partial<UpdateProfileSubscriptionEntity>
  ): Promise<ProfileSubscriptionEntity> {
    try {
      this.logger.info("Updating profile subscription", { id, subscription });

      const updateData =
        SupabaseBackendSubscriptionMapper.updateProfileSubscriptionEntityToSchema(
          subscription
        );

      const result =
        await this.dataSource.update<ProfileSubscriptionSchemaRecord>(
          "profile_subscriptions",
          id,
          updateData
        );
  ...
  async cancelSubscription(id: string): Promise<ProfileSubscriptionEntity> {
    try {
      ...
      const updatedData = await this.dataSource.update<
        Record<string, unknown> & ProfileSubscriptionSchema
      >("profile_subscriptions", id, updateData);
