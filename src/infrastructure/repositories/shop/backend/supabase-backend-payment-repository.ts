import {
  ...
  PaymentStatsEntity,
} from "@/src/domain/entities/shop/backend/backend-payment.entity";
import {
  DatabaseDataSource,
  FilterOperator,
  QueryOptions,
...
import {
  PaymentMethodStatsSchema,
  PaymentSchema,
  PaymentStatsSchema,
} from "@/src/infrastructure/schemas/shop/backend/payment.schema";
...
type PaymentMethodStatsSchemaRecord = Record<string, unknown> &
  PaymentMethodStatsSchema;
 ...
 */
export class SupabaseShopBackendPaymentRepository
  extends StandardRepository
  implements ShopBackendPaymentRepository
{
  ...
  async getPaginatedPayments(
    ...
    try {
      ...
      if (payments.length === 0) {
        ...
        const pagination =
          SupabaseShopBackendPaymentMapper.createPaginationMeta(
            page,
            limit,
            totalItems
          );
      ...
      const queues = await this.dataSource.getAdvanced<QueueRecord>(
        "queues",
        queuesQuery
      );
      ...
      const customerIds = [
        ...new Set(queues.map((q) => q.customer_id).filter(Boolean)),
      ];
      const shopIds = [
        ...new Set(queues.map((q) => q.shop_id).filter(Boolean)),
      ];
      const servedByEmployeeIds = [
        ...new Set(queues.map((q) => q.served_by_employee_id).filter(Boolean)),
      ];
      const processedByEmployeeIds = [
        ...new Set(
          payments.map((p) => p.processed_by_employee_id).filter(Boolean)
        ),
      ];
      const allEmployeeIds = [
        ...new Set([...servedByEmployeeIds, ...processedByEmployeeIds]),
      ];

      // Parallel queries for better performance
      const [customers, shops, employees] = await Promise.all([
        customerIds.length > 0
          ? this.dataSource.getAdvanced<CustomerRecord>("customers", {
              select: ["id", "name"],
              filters: [
                {
                  field: "id",
          ...
          : [],
        shopIds.length > 0
          ? this.dataSource.getAdvanced<ShopRecord>("shops", {
              select: ["id", "name"],
              filters: [
                { field: "id", operator: FilterOperator.IN, value: shopIds },
              ],
            })
          : [],
        allEmployeeIds.length > 0
          ? this.dataSource.getAdvanced<EmployeeRecord>("employees", {
              select: ["id", "name"],
              filters: [
                {
                  field: "id",
      ...
      const mappedPayments = payments.map((payment) => {
        ...
        const processedByEmployee = payment.processed_by_employee_id
          ? employeeMap.get(payment.processed_by_employee_id)
          : null;
      ...
      // Get total count
      const totalItems = await this.dataSource.count("payments");
      const pagination = SupabaseShopBackendPaymentMapper.createPaginationMeta(
        page,
        limit,
        totalItems
      );
  ...
  async getPaymentStats(): Promise<PaymentStatsEntity> {
    try {
      ...
      const statsData =
        await this.dataSource.getAdvanced<PaymentStatsSchemaRecord>(
  ...
  async getPaymentMethodStats(): Promise<PaymentMethodStatsEntity> {
    try {
      ...
      const methodStatsData =
        await this.dataSource.getAdvanced<PaymentMethodStatsSchemaRecord>(
  ...
  async getPaymentById(id: string): Promise<PaymentEntity | null> {
    try {
      ...
      const payment = await this.dataSource.getById<PaymentSchemaRecord>(
        "payments",
        id
      );
      ...
      const queue = await this.dataSource.getById<QueueRecord>(
        "queues",
        payment.queue_id
      );
      ...
      const [customer, shop, processedByEmployee] = await Promise.all([
        queue.customer_id
          ? this.dataSource.getById<CustomerRecord>(
              "customers",
              queue.customer_id
            )
          : null,
        queue.shop_id
          ? this.dataSource.getById<ShopRecord>("shops", queue.shop_id)
          : null,
        payment.processed_by_employee_id
          ? this.dataSource.getById<EmployeeRecord>(
              "employees",
              payment.processed_by_employee_id
            )
          : null,
   ...
   */
  async createPayment(
    payment: Omit<CreatePaymentEntity, "id" | "createdAt" | "updatedAt">
    ...
    try {
      ...
      const paymentSchema = {
        ...
        processed_by_employee_id: payment.processedByEmployeeId || undefined,
      };
  ...
  async updatePayment(
    id: string,
    payment: Partial<Omit<PaymentEntity, "id" | "createdAt" | "updatedAt">>
  ): Promise<PaymentEntity> {
    try {
      ...
      if (payment.queueId !== undefined)
        paymentSchema.queue_id = payment.queueId;
      if (payment.queueNumber !== undefined)
        paymentSchema.queue_number = payment.queueNumber;
      if (payment.customerName !== undefined)
        paymentSchema.customer_name = payment.customerName;
      if (payment.totalAmount !== undefined)
        paymentSchema.total_amount = payment.totalAmount;
      if (payment.paidAmount !== undefined)
        paymentSchema.paid_amount = payment.paidAmount;
      if (payment.paymentMethod !== undefined)
        paymentSchema.payment_method = payment.paymentMethod;
      if (payment.paymentStatus !== undefined)
        paymentSchema.payment_status = payment.paymentStatus;
      if (payment.paymentDate !== undefined)
        paymentSchema.payment_date = payment.paymentDate;
      if (payment.processedByEmployeeId !== undefined)
        paymentSchema.processed_by_employee_id = payment.processedByEmployeeId;
  ...
  async deletePayment(id: string): Promise<boolean> {
    try {
      ...
      await this.dataSource.delete("payments", id);
