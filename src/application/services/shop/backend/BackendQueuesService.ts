...
import {
  AssignQueueToEmployeeInput,
  AssignQueueToEmployeeUseCase,
} from "@/src/application/usecases/shop/backend/queues/AssignQueueToEmployeeUseCase";
...
import {
  GetNextQueueToServeInput,
  GetNextQueueToServeUseCase,
} from "@/src/application/usecases/shop/backend/queues/GetNextQueueToServeUseCase";
...
import {
  ...
  QueuePositionInfo,
} from "@/src/application/usecases/shop/backend/queues/GetQueuePositionUseCase";
import {
  GetQueuesPaginatedInput,
  GetQueuesPaginatedUseCase,
} from "@/src/application/usecases/shop/backend/queues/GetQueuesPaginatedUseCase";
...
} from "@/src/application/usecases/shop/backend/queues/UpdateQueueStatusUseCase";
...
export interface IShopBackendQueuesService {
  getQueuesData(
    ...
    page?: number,
    perPage?: number,
    filters?: GetQueuesPaginatedInput["filters"]
  ): Promise<QueuesDataDTO>;
  getQueuesPaginated(
    page: number,
    limit: number,
    filters?: GetQueuesPaginatedInput["filters"]
  ): Promise<PaginatedQueuesDTO>;
  getQueueById(id: string): Promise<QueueDTO>;
  createQueue(queueData: CreateQueueInput): Promise<QueueDTO>;
  updateQueue(
    id: string,
    queueData: Omit<UpdateQueueInput, "id">
  ): Promise<QueueDTO>;
  ...
  getNextQueueToServe(
    input: GetNextQueueToServeInput
  ): Promise<QueueDTO | null>;
...
export class ShopBackendQueuesService implements IShopBackendQueuesService {
  constructor(
    private readonly getQueuesPaginatedUseCase: IUseCase<
      GetQueuesPaginatedInput,
      PaginatedQueuesDTO
    >,
    ...
    private readonly assignQueueToEmployeeUseCase: IUseCase<
      AssignQueueToEmployeeInput,
      QueueDTO
    >,
    private readonly updateQueueStatusUseCase: IUseCase<
      UpdateQueueStatusInput,
      QueueDTO
    >,
    private readonly getNextQueueToServeUseCase: IUseCase<
      GetNextQueueToServeInput,
      QueueDTO | null
    >,
    private readonly getQueuePositionUseCase: IUseCase<
      GetQueuePositionInput,
      QueuePositionInfo
    >,
  ...
  async getQueuesData(
    ...
    try {
      ...
      this.logger.error("Error in getQueuesData", {
        error,
        shopId,
        page,
        perPage,
   ...
   */
  async getQueuesPaginated(
    page: number,
    limit: number,
    filters?: GetQueuesPaginatedInput["filters"]
  ): Promise<PaginatedQueuesDTO> {
    try {
      this.logger.info("Getting paginated queues with filters", {
        page,
        limit,
        filters,
      });

      const result = await this.getQueuesPaginatedUseCase.execute({
        page,
        limit,
        filters,
      });
      ...
      this.logger.error("Error in getQueuesPaginated", {
        error,
        page,
        limit,
        filters,
   ...
   * @returns Updated queue DTO
   */
  async assignQueueToEmployee(
    input: AssignQueueToEmployeeInput
  ): Promise<QueueDTO> {
    try {
  ...
  async getQueuePosition(
    input: GetQueuePositionInput
  ): Promise<QueuePositionInfo> {
    try {
...
export class ShopBackendQueuesServiceFactory {
  static create(
    repository: ShopBackendQueueRepository,
    logger: Logger
  ): ShopBackendQueuesService {
    const getQueuesPaginatedUseCase = new GetQueuesPaginatedUseCase(
      repository,
      logger
    );
    ...
    const assignQueueToEmployeeUseCase = new AssignQueueToEmployeeUseCase(
      repository,
      logger
    );
    const updateQueueStatusUseCase = new UpdateQueueStatusUseCase(
      repository,
      logger
    );
    const getNextQueueToServeUseCase = new GetNextQueueToServeUseCase(
      repository,
      logger
    );
    const getQueuePositionUseCase = new GetQueuePositionUseCase(
      repository,
      logger
    );
