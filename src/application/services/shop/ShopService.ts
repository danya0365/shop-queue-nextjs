import type {
  CreateShopInputDTO,
  GetShopsPaginatedInput,
  PaginatedShopsDTO,
  ShopDTO,
  ShopStatsDTO,
  ShopsDataDTO,
  UpdateShopInputDTO,
  UpdateShopStatusInputDTO,
...
export class ShopService implements IShopService {
  constructor(
    private readonly getShopsPaginatedUseCase: IUseCase<
      GetShopsPaginatedInput,
      PaginatedShopsDTO
    >,
  ...
  async getShopsPaginated(
    ...
    try {
      ...
      const result = await this.getShopsPaginatedUseCase.execute({
        page,
        limit: perPage,
      });
      return result;
    } catch (error) {
      this.logger.error("Error getting paginated shops data", {
        error,
        page,
        perPage,
      });
  ...
  async createShop(params: CreateShopInputDTO): Promise<ShopDTO> {
    try {
      this.logger.info("Creating shop", {
        params: { ...params, email: params.email ? "[REDACTED]" : undefined },
      });

      const result = await this.createShopUseCase.execute(params);
      return result;
    } catch (error) {
      this.logger.error("Error creating shop", {
        error,
        params: { ...params, email: params.email ? "[REDACTED]" : undefined },
      });
      throw error;
  ...
  async updateShop(params: UpdateShopInputDTO): Promise<ShopDTO> {
    try {
      ...
      this.logger.error("Error updating shop", {
        error,
        params: { ...params, email: params.email ? "[REDACTED]" : undefined },
      });
      throw error;
  ...
  async updateShopStatus(params: UpdateShopStatusInputDTO): Promise<ShopDTO> {
    try {
      this.logger.info("Updating shop status", {
      ...
      this.logger.error("Error updating shop status", {
        error,
        params: { id: params.id, status: params.status },
      });
      throw error;
...
export class ShopServiceFactory {
  static create(
    repository: ShopBackendShopRepository,
    logger: Logger
  ): ShopService {
