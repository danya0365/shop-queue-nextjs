  ...
  async execute(shopId: string): Promise<RecentActivityDTO[]> {
    try {
      ...
      const recentActivities =
        await this.dashboardRepository.getRecentActivities(shopId);

      // Map domain entities to DTOs
      const activities: RecentActivityDTO[] = recentActivities.map(
        (activity) => ({
          id: activity.id,
          type: activity.type,
      ...
      this.logger.error(
        "GetRecentActivitiesUseCase: Error retrieving recent activities",
        error
      );
      throw error;
    }
