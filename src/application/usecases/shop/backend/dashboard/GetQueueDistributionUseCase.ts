...
export class GetQueueDistributionUseCase
  implements IGetQueueDistributionUseCase
{
  constructor(
  ...
  async execute(shopId: string): Promise<QueueStatusDistributionDTO> {
    try {
      ...
      const queueDistribution =
        await this.dashboardRepository.getQueueDistribution(shopId);
      ...
      this.logger.error(
        "GetQueueDistributionUseCase: Error retrieving queue distribution",
        error
      );
      throw error;
    }
