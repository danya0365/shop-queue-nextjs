 ...
 */
export class AssignQueueToEmployeeUseCase
  implements IUseCase<AssignQueueToEmployeeInput, QueueDTO>
{
  constructor(
  ...
  async execute(input: AssignQueueToEmployeeInput): Promise<QueueDTO> {
    try {
      ...
      if (!["waiting", "confirmed"].includes(queue.status)) {
        throw new ShopBackendQueueError(
          ShopBackendQueueErrorType.VALIDATION_ERROR,
          `Cannot assign queue with status ${queue.status}. Only waiting or confirmed queues can be assigned.`,
          "assignQueueToEmployee",
